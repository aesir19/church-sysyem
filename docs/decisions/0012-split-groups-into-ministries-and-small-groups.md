# ADR-0012 — Groups are two things: ministries and small groups

**Status:** Accepted · **Date:** 2026-08-14 · **Migration:** `0026_split_groups`
...
(`GROUP_TABLES`, `MEMBERSHIP_TABLES`, `MEMBERSHIP_PARENT_KEY`).

## Consequences

- A group id alone does not say which table it lives in; code resolves the type first
  (see `fetchGroup` in `src/lib/data/group.js`).
